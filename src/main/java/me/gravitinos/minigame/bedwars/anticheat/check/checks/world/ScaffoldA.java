package me.gravitinos.minigame.bedwars.anticheat.check.checks.world;
...
import me.gravitinos.minigame.gamecore.util.PacketEvent;
...
public class ScaffoldA extends Check {
    ...
    public static void loadTrainingExamples(){
        ...
        File folder = new File(SpigotMinigames.instance.getDataFolder() + File.separator + exFolder);
        if (!folder.exists()) {
            return;
        }
...
//    }
    ...
    private synchronized boolean check() {
        ...
        if (learningFrom) {
            ...
            File file = new File(SpigotMinigames.instance.getDataFolder() + File.separator + exFolder, UUID.randomUUID().toString() + ".yml");
            if (!file.exists()) {
                try {
