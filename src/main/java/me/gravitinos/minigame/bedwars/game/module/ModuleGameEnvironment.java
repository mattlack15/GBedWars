...
public class ModuleGameEnvironment extends GameModule {
    ...
    public ModuleGameEnvironment(GameHandler handler, @NotNull CuboidRegion region) {
        super(handler, "GAME_ENVIRONMENT");
        this.region = region;
    ...
    public void enable() {
        ...
        currentTaskId = getTask().runTaskTimer(CoreHandler.main, 0, 10).getTaskId();
    }
    ...
    private void onLiquidPlace(PlayerBucketEmptyEvent event) {
        ...
        if (getGameHandler().isRunning()) {
            ...
            }.runTask(SpigotMinigames.instance);
        }
    }
    ...
    private void onInventoryClick(InventoryClickEvent event) {
        ...
        if (getGameHandler().isPlaying(event.getWhoClicked().getUniqueId())) {
            if (event.getSlotType() == InventoryType.SlotType.ARMOR || (event.getCurrentItem() != null && event.getCurrentItem().getType().toString().contains("PICKAXE"))) {
                event.setCancelled(true);
                return;
            }
        }
    ...
    private void onDrop(ItemSpawnEvent event) {
        ...
        if (event.getEntity().getItemStack().getType().equals(Material.BED) ||
                event.getEntity().getItemStack().getType().equals(Material.RED_ROSE) ||
                event.getEntity().getItemStack().getType().equals(Material.SEEDS)) {
            event.setCancelled(true);
        }
    }
    ...
    private void onFlow(BlockFromToEvent event) {
        if (!this.isEnabled())
            return;
        this.revert.add(event.getToBlock().getState());
    }
    ...
    private void onPlace(BlockPlaceEvent event) {
        ...
        if (getGameHandler().isRunning()) {
            ...
            if (event instanceof BlockMultiPlaceEvent) {
                ((BlockMultiPlaceEvent) event).getReplacedBlockStates().forEach(s -> {
                    s.getBlock().setMetadata(PLAYER_PLACED_BLOCK_META, new FixedMetadataValue(CoreHandler.main, true));
                    revert.add(s);
                });
            } else {
