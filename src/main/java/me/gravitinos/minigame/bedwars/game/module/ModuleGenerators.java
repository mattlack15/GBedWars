...
import me.gravitinos.minigame.bedwars.game.module.generator.Generator;
import me.gravitinos.minigame.bedwars.game.module.generator.GeneratorDrop;
...
public class ModuleGenerators extends GameModule {
    ...
    private static final double OUTER_GENERATOR_INTERVAL = 25;
    ...
    public ModuleGenerators(@NotNull GameHandler handler, ArrayList<Location> midGens, ArrayList<Location> outerGens, ArrayList<Location> baseGens) {
        super(handler, "GENERATORS");
    ...
    private BukkitRunnable getTask() {
        return //Task interval of 1 tick or 0.05s
                new BukkitRunnable() {
                    int i = 0;
                    ...
                    public void run() {
                        ...
                        for (Generator generator : generators) {
                            ...
                            boolean maxHeld = !(generator.getHeldItems() / generator.getDrops().size() < generator.getMaxHeldItems());

                            if (maxHeld) {
                                if(box.getLines().size() > 0 ){
                                    box.setLine(0, "&c&lWaiting...");
                                }
                            } else {
                                //Decrement countdown
    ...
    public void enable() {
        super.enable();
        this.enabled = true;
        currentTaskId = getTask().runTaskTimer(CoreHandler.main, 0, 1).getTaskId();
    }
