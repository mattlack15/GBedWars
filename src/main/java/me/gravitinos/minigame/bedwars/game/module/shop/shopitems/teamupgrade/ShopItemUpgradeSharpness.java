    ...
    public @NotNull Supplier<ItemStack> getDisplayItemSupplier(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return () -> {
            ...
            if(nextLevel < upgrade.getMaxLevel()){
                ...
                return new ItemBuilder(Material.DIAMOND_SWORD, 1).setName("&b&lSharpness").addLore("&cMax Level Reached").addItemFlags(ItemFlag.HIDE_ATTRIBUTES).build();
            }
            return ShopItem.addPrice(new ItemBuilder(Material.DIAMOND_SWORD, 1).setName("&b&lSharpness &a" + nextLevel).addItemFlags(ItemFlag.HIDE_ATTRIBUTES).build(), getCost(playerInfo, gameItems));
        };
    }
