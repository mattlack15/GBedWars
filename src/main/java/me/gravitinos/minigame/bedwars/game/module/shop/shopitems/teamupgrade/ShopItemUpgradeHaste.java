...
public class ShopItemUpgradeHaste extends ShopItemData {

    private static final TeamUpgrade upgrade = TeamUpgrade.HASTE;
    ...
    public @NotNull Supplier<ItemStack> getDisplayItemSupplier(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return () -> {
            ...
            if(nextLevel < upgrade.getMaxLevel()){
                ...
                return new ItemBuilder(Material.IRON_PICKAXE, 1).setName("&e&lHaste").addLore("&cMax Level Reached").addItemFlags(ItemFlag.HIDE_ATTRIBUTES).build();
            }
            return ShopItem.addPrice(new ItemBuilder(Material.IRON_PICKAXE, 1).setName("&e&lHaste &a" + nextLevel).addItemFlags(ItemFlag.HIDE_ATTRIBUTES).build(), getCost(playerInfo, gameItems));
        };
    }
