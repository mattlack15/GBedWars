...
import me.gravitinos.minigame.bedwars.game.module.ModuleGameEnvironment;
...
public class ItemExplosiveBow extends SimpleGameItemHandler {
    ...
    public void onDisable() {
        for(UUID ids : this.fallingBlocks){
            Entity ent = Bukkit.getEntity(ids);
    ...
    private void onLand(ProjectileHitEvent event){
        if(event.getEntity().hasMetadata(META_VAL)){
            ...
            this.fallingBlocks.addAll(createExplosion(event.getEntity().getLocation(), (int) 3.5, (Entity) event.getEntity()));
    ...
    public static ArrayList<UUID> createExplosion(Location center, int radius, Entity damager){
        ...
        for(Entity ents : center.getWorld().getNearbyEntities(center, radius, radius, radius)){
        ...
        ArrayList<Block> states1 = getBlocksSphere(center, radius);
