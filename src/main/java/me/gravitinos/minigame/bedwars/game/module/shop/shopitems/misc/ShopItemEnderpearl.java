...
public class ShopItemEnderpearl extends ShopItemData {
    @Override
    public @NotNull Consumer<Player> getGiver(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return (p) -> p.getInventory().addItem(gameItems.getGameItem(BedwarsItem.ITEM_ENDERPEARL.toString()).getItem(1));
    }

    @Override
    public @NotNull Supplier<ItemStack> getDisplayItemSupplier(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return () -> gameItems.getGameItem(BedwarsItem.ITEM_ENDERPEARL.toString()).getItem(1);
    }
