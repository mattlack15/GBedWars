    ...
    public @NotNull Supplier<ItemStack> getDisplayItemSupplier(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return () -> {
            ...
            if(nextLevel < upgrade.getMaxLevel()){
                ...
                return new ItemBuilder(Material.IRON_PICKAXE, 1).setName("&e&lHaste").addLore("&c&lMax Level Reached").build();
            }
            return ShopItem.addPrice(new ItemBuilder(Material.IRON_PICKAXE, 1).setName("&e&lHaste &a" + nextLevel).build(), getCost(playerInfo, gameItems));
        };
    }
