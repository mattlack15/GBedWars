    ...
    public @NotNull Supplier<ItemStack> getDisplayItemSupplier(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return () -> {
            ...
            if(nextLevel < upgrade.getMaxLevel()){
                ...
                return new ItemBuilder(Material.DIAMOND_CHESTPLATE, 1).setName("&6&lProtection").addLore("&cMax Level Reached").build();
            }
            return ShopItem.addPrice(new ItemBuilder(Material.DIAMOND_CHESTPLATE, 1).setName("&6&lProtection &a" + nextLevel).build(), getCost(playerInfo, gameItems));
        };
    }
