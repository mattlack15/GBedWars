...
public class ShopItemExplosiveBow extends ShopItemData {
    @Override
    public @NotNull Consumer<Player> getGiver(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return (p) -> p.getInventory().addItem(new ItemBuilder(gameItems.getGameItem(BedwarsItem.ITEM_EXPLOSIVE_BOW.toString()).getItem(1)).build());
    }

    @Override
    public @NotNull Supplier<ItemStack> getDisplayItemSupplier(BWPlayerInfo playerInfo, ModuleGameItems gameItems) {
        return () -> gameItems.getGameItem(BedwarsItem.ITEM_EXPLOSIVE_BOW.toString()).getItem(1);
    }
