...
public abstract class MapKeyPoint {
    ...
    public void build(EditSession session, ArrayList<Location> locations) throws MaxChangedBlocksException {
        for(Location locs : locations){
            for(Vector vecs : identifyingPoints.keySet()) {
                ...
                session.setBlock(new com.sk89q.worldedit.Vector(vec2.getX(), vec2.getY(), vec2.getZ()), new BaseBlock(identifyingPoints.get(vecs).getId(), identifyingPoints.get(vecs).getData()));
            }
        }
    }
    ...
    public void place(EditSession session, Vector location){
        //TODO
    }
}
